...
Updates (v2):
  * Column order reversed: leftmost = newest game (G3 5/8), rightmost = oldest (G1 5/4)
  * Cam Payne removed (waived 4/2026), replaced by Dalen Terry on 76ers roster
  * Box scores expanded with verified stats from ESPN/NBA.com/RotoWire/CBS Sports/
    FanDuel Research/Posting and Toasting/Liberty Ballers/SI/Yahoo (all cross-checked
    via WebSearch snippets — direct WebFetch returned 403 on all primary sources)
  * Cells where 2-source confirmation was unavailable are marked "—" (no fake numbers)
"""
...
NYK_PLAYERS = [
    ...
    (10, "ジェレミー・ソーチャン", "PF", 203, 104, "2003-05-20", 3, "ベイラー大出身。2/13にスパーズから加入、フロントコート補強"),
]
...
PHI_PLAYERS = [
    ...
    (4, "デイレン・テリー", "SG", 198, 88, "2002-07-12", 3, "アリゾナ大出身。4月にPaynは契約解除→本契約に昇格"),
    (22, "ジョニ・ブルーム", "C", 208, 109, "2002-07-19", 0, "オーバーン大出身、2巡指名ルーキー、半月板手術明け"),
]


# ============================================================================
# BOX SCORES — column order REVERSED: leftmost = newest game
#   G1 column = "Game 3" (5/8) — newest
#   G2 column = "Game 2" (5/6) — middle
#   G3 column = "Game 1" (5/4) — oldest
# Each value: [MIN, PTS, 3PM, 3PA, AST, REB] OR "DNP" / "OUT"
# Use "—" inside the list for individual unverified cells.
# ============================================================================

NYK_BOX = {
    "G1_label": "シリーズ第3戦 5/8 ○108-94 @PHI",
    "G2_label": "シリーズ第2戦 5/6 ○108-102 vs PHI",
    "G3_label": "シリーズ第1戦 5/4 ○137-98 vs PHI",
    "data": {
        # 順序: [G3(5/8新), G2(5/6中), G1(5/4旧)]
        "ジェイレン・ブランソン":     [[38, 33, 3, 8, 9, 5], [41, 26, 1, 5, 6, 1], [31, 35, 3, 6, 3, 1]],
        "マイケル・ブリッジス":       [[37, 23, 2, 4, 1, 3], [38, 18, "—", "—", "—", 5], [27, 17, 3, 5, 5, 2]],
        "OG・アヌノビー":             ["OUT",                [37, 24, 2, 7, 2, 5], [30, 18, 2, 2, 1, 3]],
        "ジョシュ・ハート":           [[40, 12, 0, 4, 3, 11], [44, 5, 1, 5, 6, 7], [26, 8, 1, 2, 6, 8]],
        "カール=アンソニー・タウンズ": [[26, 8, 0, 2, 7, 12], [27, 20, 1, 1, 7, 10], [20, 17, 3, 5, 6, 6]],
        "ミッチェル・ロビンソン":     [[19, 6, 0, 0, "—", 6], "OUT",                [12, 2, 0, 0, 1, 4]],
        "マイルズ・マクブライド":     [[21, 3, 1, 5, 2, "—"], [21, 4, 1, 4, 1, 1], [10, 8, 2, 2, 4, "—"]],
        "ホセ・アルバラード":         [["—", 3, 1, "—", "—", "—"], [7, 3, 1, 2, 1, 0], [9, 5, "—", "—", 1, 1]],
        "ランドリー・シャメット":     [[26, 15, 2, 3, "—", 3], [8, 0, 0, "—", "—", "—"], [9, 5, 1, 3, 1, 1]],
        "ジョーダン・クラークソン":   [["—", 4, "—", "—", 3, 5], [8, 6, 0, 1, "—", 5], [12, 3, 1, 3, 2, "—"]],
        "アリエル・フクポルティ":     ["DNP",                [7, 2, 0, 0, "—", 3], [17, 5, 0, 0, "—", 9]],
        "タイラー・コレック":         ["DNP",                "DNP",                ["—", 8, "—", "—", 4, "—"]],
        "パコメ・ダディエ":           ["DNP",                "DNP",                ["—", 4, "—", "—", "—", 1]],
        "モハメド・ディアワラ":       ["DNP",                "DNP",                [8, 5, 1, 2, 1, 2]],
        "ジェレミー・ソーチャン":     ["DNP",                [1, 0, 0, 0, 0, 0],   "DNP"],
    }
}

PHI_BOX = {
    "G1_label": "シリーズ第3戦 5/8 ●94-108 vs NYK",
    "G2_label": "シリーズ第2戦 5/6 ●102-108 @NYK",
    "G3_label": "シリーズ第1戦 5/4 ●98-137 @NYK",
    "data": {
        "タイリース・マクシー":       [[44, 17, 1, 3, 7, 2],  [47, 26, 1, 6, 6, 3],   [27, 13, 0, 3, 2, 3]],
        "VJ・エッジコム":             [["—", 11, "—", "—", 4, 7], [40, 17, 3, 7, 3, 5], [28, 12, 2, 5, 2, 1]],
        "ケリー・オウブレJr.":         [[36, 22, 2, 5, 1, 8],  [40, 19, 3, 6, 2, 5],   ["—", 12, 1, 2, 1, 5]],
        "ポール・ジョージ":           [[38, 15, 3, "—", 3, 5],[43, 19, 5, 13, 4, 6],  [26, 17, 4, 6, 3, 3]],
        "ジョエル・エンビード":       [[35, 18, 0, 4, 5, 6],  "OUT",                  [25, 14, 0, 2, 1, 4]],
        "クエンティン・グライムス":   [[22, 6, 2, "—", 2, 2], [24, 7, 1, 2, 3, 3],    ["—", 3, "—", "—", "—", "—"]],
        "アンドレ・ドラモンド":       [[3, "—", 0, 0, "—", "—"],[15, 6, 0, 0, "—", 8],["—", "—", 0, 0, "—", "—"]],
        "ジャスティン・エドワーズ":   ["DNP",                 "DNP",                  ["—", 4, 1, "—", "—", "—"]],
        "アデム・ボナ":               [[1, "—", 0, 0, "—", "—"], [16, 2, 0, 0, 1, 7], [4, 0, 0, 0, 0, "—"]],
        "ドミニック・バーロウ":       [["—", "—", "—", "—", "—", "—"], [16, 6, 0, 0, "—", 2], ["—", 2, "—", "—", "—", "—"]],
        "トレンドン・ワトフォード":   [[1, "—", "—", "—", "—", "—"], ["—", 1, "—", "—", "—", "—"], ["—", 1, "—", "—", "—", "—"]],
        "デイレン・テリー":           [["—", "—", "—", "—", "—", "—"], "DNP",         "DNP"],
...
def main():
    ...
    # Title row
    ws.cell(row=1, column=1, value="2026 NBAプレーオフ 第2回戦・第4戦  フィラデルフィア・76ers @ ニューヨーク・ニックス  2026/5/10(現地)・JST 5/11 4:30 タップオフ ／ シリーズ 3勝0敗（NYKリード）  ※ G列順は新しい試合（G3=5/8）が左").font = Font(name="MS Gothic", size=12, bold=True)
